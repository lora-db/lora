...
`lora-database` Criterion suite with bencher output and publishes both the
raw archive and `benchmark-summary.json`. The JSON summary is generated by
`scripts/summarize-benchmarks.mjs` and includes per-benchmark timings,
group rollups, fastest/slowest lists, runner metadata, and optional
baseline comparisons when a baseline is supplied.
