...
//!
//! Owned by the analyzer because the analyzer validates function
//! references at compile time. The executor consults the same
//! `op` strings; drift-safety tests in lora-executor assert that every
//! entry here has a dispatch arm.
//!
//! This is intentionally more than an arity table: enum-like argument slots
//! live here too, so analyzer rewrites and executor dispatch share one
