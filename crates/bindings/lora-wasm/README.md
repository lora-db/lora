...
yarn workspace @loradb/lora-wasm build   # wasm-pack (node + bundler + web) + tsc
yarn workspace @loradb/lora-wasm test    # vitest
```
...
The workspace-level `package.json` is the source of truth for the npm
package name, version, exports, and publish settings. The `pkg-*`
directories are generated by `wasm-pack`; if their nested manifests look
stale after a version bump, rebuild with
`yarn workspace @loradb/lora-wasm build` before testing or packing.

To run the full validation suite (typecheck, vitest, Playwright browser
test, npm pack dry-run), add:

```bash
yarn workspace @loradb/lora-wasm typecheck
yarn workspace @loradb/lora-wasm test
yarn workspace @loradb/lora-wasm test:browser:install   # one-time chromium download
yarn workspace @loradb/lora-wasm test:browser
yarn workspace @loradb/lora-wasm pack:dry
```
...
- The non-blocking worker-backed path is end-to-end verified: a real
  ...
  behind `yarn workspace @loradb/lora-wasm test:browser`.
...
- Run `yarn workspace @loradb/lora-wasm pack:dry` after every version
  bump. It regenerates the `pkg-*` outputs, copies the root license, and
  shows the exact files that would ship to npm.
- Check the three wasm bundles before release. They are each about
  2.2 MB today; the package uses conditional exports so consumers import
  only the target they need, but CDN/browser deployments may still want
  compressed `.wasm` served with `Content-Encoding: br` or `gzip`.
