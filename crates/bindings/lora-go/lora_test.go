...
func TestParseErrorReturnsLoraError(t *testing.T) {
	...
	if lerr.Code != lora.CodeParse {
		t.Fatalf("Code = %q, want LORA_PARSE", lerr.Code)
	}
	if !lerr.IsClient() {
		t.Fatalf("parse error should be a client error")
	}
}
...
func TestErrorsIsMatchesOnCode(t *testing.T) {
	db := newDB(t)
	_, err := db.Execute("NOT CYPHER", nil)
	if !errors.Is(err, &lora.LoraError{Code: lora.CodeParse}) {
