 ...
 *
 * `decodeQuery` returns `null` for any malformed input rather than throwing,
 * because share links are user-supplied data and we never want a bad paste
 * to crash the workbench.
 */
...
/** Parses a `"#q=...&snap=..."`-style hash fragment into its known fields. */
export function readHash(hash: string): { q?: string; snap?: string } {
  if (typeof hash !== "string" || hash.length === 0) return {};
  // Tolerate both `#a=b&c=d` and `a=b&c=d`.
  const trimmed = hash.startsWith("#") ? hash.slice(1) : hash;
  if (trimmed.length === 0) return {};

  const out: { q?: string; snap?: string } = {};
