...
export default function BenchmarkSummary({
  src = "/benchmarks/perf-smoke-summary.json",
}) {
  ...
  const sortedRows = useMemo(
    () =>
      [...rows].sort(
        (a, b) => (b.baseline?.ratio ?? 0) - (a.baseline?.ratio ?? 0),
      ),
    [rows],
  );
  ...
  return (
    <section
      ...
      aria-label="Benchmark summary loaded from JSON"
    >
          ...
          <tbody>
            {sortedRows.map((bench) => (
              <tr key={bench.name}>
                ...
                <td>
                  <span
                    className={`${styles.status} ${statusClass(bench.baseline?.status)}`}
                  >
