...
<CypherSnippet code={String.raw`MATCH (n)                           // every node
...
<CypherSnippet code={String.raw`WHERE p.born >= 1900 AND p.born < 1950
...
<CypherSnippet code={String.raw`CREATE INDEX user_email FOR (u:User) ON (u.email)
...
<QueryCodeBlock code={String.raw`CREATE CONSTRAINT user_email FOR (u:User) REQUIRE u.email IS UNIQUE;
...
<QueryCodeBlock code={String.raw`RETURN p.name, p.born;
...
<CypherSnippet code={String.raw`WITH u, count(p) AS posts              // group + pipe forward
...
<CypherSnippet code={String.raw`ORDER BY n.name ASC
ORDER BY count(*) DESC
...
<CypherSnippet code={String.raw`count(*)                               // rows
...
<CypherSnippet code={String.raw`CASE WHEN x >= 50 THEN 'ok' ELSE 'low' END
...
<QueryCodeBlock code={String.raw`MATCH p = (a)-[:R*1..3]->(b)           // 1 to 3 hops
...
<CypherSnippet code={String.raw`string.lower(s), string.upper(s)
...
<CypherSnippet code={String.raw`math.abs(x), math.ceil(x), math.floor(x), math.round(x)
...
<CypherSnippet code={String.raw`value.size(xs), list.first(xs), list.rest(xs), list.last(xs)
...
<CypherSnippet code={String.raw`'2026-04-20'::DATE
'12:00:00'::TIME
...
<CypherSnippet code={String.raw`{x: 1, y: 2}::POINT                                   // Cartesian 2D
...
<CypherSnippet code={String.raw`type.of(x)                           // 'INTEGER', 'STRING', 'NODE', …
...
<CypherSnippet code={String.raw`id(n), id(r)
labels(n), type(r)
keys(n), properties(n)
