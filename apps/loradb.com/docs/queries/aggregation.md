...
<QueryCodeBlock code={String.raw`MATCH (:LabelThatDoesNotExist)
RETURN count(*)   // 0`} />
...
<QueryCodeBlock code={String.raw`MATCH (p:Person)-[:VISITED]->(c:City)
...
<QueryCodeBlock code={String.raw`MATCH (p:Person)-[:BOUGHT]->(b:Book)
RETURN p.name,
...
<QueryCodeBlock code={String.raw`MATCH (u:User)-[:PLACED]->(o:Order)
...
<QueryCodeBlock code={String.raw`MATCH (a:Author)-[:WROTE]->(:Book)-[:IN]->(g:Genre)
