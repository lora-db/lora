...
<QueryCodeBlock code={String.raw`UNWIND [1, 2, 3] AS n RETURN n
;// 1, 2, 3
...
<QueryCodeBlock code={String.raw`UNWIND [1, 2, 2, 3, 3, 3] AS x
RETURN collect(DISTINCT x)
// [1, 2, 3]`} />

### Chain UNWINDs

<QueryCodeBlock code={String.raw`UNWIND $groups AS g
UNWIND g.members AS m
CREATE (:Member {group: g.name, name: m})`} />

### Unwind after aggregation

<QueryCodeBlock code={String.raw`MATCH (p:Person)-[:KNOWS]->(f)
...
<QueryCodeBlock code={String.raw`UNWIND [] AS x
...
<QueryCodeBlock code={String.raw`MERGE (n:User {id: 1001}) RETURN n`} />
...
<QueryCodeBlock code={String.raw`MERGE (u:User {id: $id}) RETURN u`} />
...
<QueryCodeBlock code={String.raw`MERGE (n:User {id: 1002})
...
<QueryCodeBlock code={String.raw`// Matches only the exact pattern
...
<QueryCodeBlock code={String.raw`// Bad — 'updated_at' is part of the match key
...
<QueryCodeBlock code={String.raw`// Risky: creates Alice and Bob if either is missing
...
<QueryCodeBlock code={String.raw`UNWIND $tags AS t
...
<QueryCodeBlock code={String.raw`MERGE (c:Counter {name: 'views'})
...
<QueryCodeBlock code={String.raw`MATCH (u:User {id: $user}), (r:Role {name: $role})
MERGE (u)-[:HAS_ROLE]->(r)`} />
...
<QueryCodeBlock code={String.raw`UNWIND $readings AS v
...
<QueryCodeBlock code={String.raw`UNWIND $scores AS s
...
<QueryCodeBlock code={String.raw`// Won't match a plain :User {id: 1}
