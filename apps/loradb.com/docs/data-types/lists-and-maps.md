...
<QueryCodeBlock code={String.raw`RETURN [1, 2, 3];
RETURN ['a', 'b', 'c'];
...
<QueryCodeBlock code={String.raw`UNWIND $items AS it
...
<QueryCodeBlock code={String.raw`MATCH (u:User)-[:OWNS]->(r:Repo)
...
<QueryCodeBlock code={String.raw`RETURN {name: 'Ada', born: 1815};
...
<QueryCodeBlock code={String.raw`WITH {name: 'Ada', born: 1815} AS m
RETURN m.name, m.born, m['name']`} />

Accessing a missing key returns `null`.

<QueryCodeBlock code={String.raw`WITH {a: 1} AS m
RETURN m.b                   // null`} />
...
<QueryCodeBlock code={String.raw`RETURN {a: 1, b: 2} + {b: 20, c: 3}
// {a: 1, b: 20, c: 3}`} />
...
<QueryCodeBlock code={String.raw`CREATE (p:Person {name: 'Ada', born: 1815, skills: ['math', 'cs']});
...
<QueryCodeBlock code={String.raw`RETURN map.get({name: 'Ada'}, 'name');                    // 'Ada'
...
<QueryCodeBlock code={String.raw`WITH ['a', 'b', 'c'] AS keys, [1, 2, 3] AS vals
...
<QueryCodeBlock code={String.raw`// IN on a list
...
<QueryCodeBlock code={String.raw`WITH [1, 2, 3] AS xs, 2 AS new_x
RETURN CASE WHEN new_x IN xs THEN xs ELSE xs + new_x END
// [1, 2, 3]`} />

### Index of first match

<QueryCodeBlock code={String.raw`WITH ['a', 'b', 'c', 'd'] AS xs, 'c' AS needle
...
<QueryCodeBlock code={String.raw`WITH {a: 1, nested: {x: 1, y: 2}} AS base,
...
<QueryCodeBlock code={String.raw`RETURN value.size([]), value.size({});    // 0, 0
...
<QueryCodeBlock code={String.raw`RETURN {a: 1, b: 2} = {b: 2, a: 1}    // true`} />
