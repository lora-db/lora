...
<QueryCodeBlock code={String.raw`WITH {name: 'Ada'} AS person
RETURN person.name`} />
...
<QueryCodeBlock code={String.raw`RETURN map.pick({id: 1, name: 'Ada', email: 'ada@example.test'}, ['id', 'name'])
// {id: 1, name: 'Ada'}`} />

Introspection helpers expose a map as lists:

<QueryCodeBlock code={String.raw`RETURN map.keys({c: 1, a: 2, b: 3});       // ['a', 'b', 'c']
...
<QueryCodeBlock code={String.raw`RETURN map.set({a: 1}, 'b', 2);             // {a: 1, b: 2}
...
<QueryCodeBlock code={String.raw`RETURN map.remove({a: 1, b: 2, c: 3}, ['a', 'c'])
// {b: 2}`} />
...
<QueryCodeBlock code={String.raw`RETURN map.merge({a: 1}, {a: 9, b: 2});           // {a: 9, b: 2}
...
<QueryCodeBlock code={String.raw`RETURN map.deep_merge(
...
<QueryCodeBlock code={String.raw`RETURN map.from(['a', 1, 'b', 2]);                 // {a: 1, b: 2}
...
<QueryCodeBlock code={String.raw`RETURN map.get_path({user: {name: 'Ada'}}, 'user.name')
;// 'Ada'
...
<QueryCodeBlock code={String.raw`RETURN map.flatten({user: {name: 'Ada', age: 36}, active: true})
...
<QueryCodeBlock code={String.raw`WITH [
...
<QueryCodeBlock code={String.raw`RETURN map.invert({draft: 0, published: 1})
// {'0': 'draft', '1': 'published'}`} />
