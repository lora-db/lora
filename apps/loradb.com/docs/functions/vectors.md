...
<QueryCodeBlock code={String.raw`// Integer-backed
...
<QueryCodeBlock code={String.raw`RETURN [1, 2, 3]::VECTOR<INTEGER>(3);
RETURN [1, 2, 3]::VECTOR<INT8>(3);
RETURN [1, 2, 3]::VECTOR<FLOAT>(3)      // FLOAT aliases FLOAT64`} />

### From a parameter

<QueryCodeBlock code={String.raw`RETURN $embedding::VECTOR<FLOAT32>(384) AS query_vec;
...
<QueryCodeBlock code={String.raw`// Integers promoted to float-backed vectors (exact for small magnitudes)
...
<QueryCodeBlock code={String.raw`RETURN null::VECTOR<FLOAT32>(3);      // null
...
<QueryCodeBlock code={String.raw`// Cosine: (1 + raw_cosine) / 2
...
<QueryCodeBlock code={String.raw`// Pure VECTOR on both sides
...
<QueryCodeBlock code={String.raw`// null on either side → null
...
<QueryCodeBlock code={String.raw`WITH [1, 2, 3]::VECTOR<FLOAT32>(3) AS a,
...
<QueryCodeBlock code={String.raw`// L2 / Euclidean — generic "closeness", respects magnitude.
...
<QueryCodeBlock code={String.raw`// null vectors or null metric → null
...
<QueryCodeBlock code={String.raw`WITH [3, 4]::VECTOR<FLOAT32>(2) AS v
...
<QueryCodeBlock code={String.raw`WITH [3, 0, 4]::VECTOR<FLOAT32>(3) AS v
...
<QueryCodeBlock code={String.raw`WITH [1, 2, 3, 4]::VECTOR<FLOAT32>(4) AS v
...
<QueryCodeBlock code={String.raw`RETURN vector.coordinates([1.9, -1.9, 3.0]::VECTOR<FLOAT32>(3), INTEGER)
;       // [1, -1, 3]
...
<QueryCodeBlock code={String.raw`MATCH (d:Doc {tenant: $tenant})
...
<QueryCodeBlock code={String.raw`MATCH (seed:Doc {id: $seed_id})-[:SIMILAR_TO*1..2]-(candidate:Doc)
...
<QueryCodeBlock code={String.raw`UNWIND $queries AS q
MATCH (d:Doc)
...
<QueryCodeBlock code={String.raw`WITH [$q1, $q2, $q3] AS qs
MATCH (d:Doc)
...
<QueryCodeBlock code={String.raw`WITH [0.10, 0.20, 0.30]::VECTOR<FLOAT32>(3) AS q
MATCH (d:Doc {id: $id})
RETURN d.id,
...
<QueryCodeBlock code={String.raw`// Works, but meaningless as a primary sort.
...
<QueryCodeBlock code={String.raw`RETURN vector.similarity([1,2,3]::VECTOR<INT8>(3),
                                [2,4,6]::VECTOR<INT8>(3))
